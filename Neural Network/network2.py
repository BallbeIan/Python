...
try:
    loaded_weights = import_weights()

    weights = {
        'W1': np.array(loaded_weights['W1']),
        'b1': np.array(loaded_weights['b1']),
        'W2': np.array(loaded_weights['W2']),
        'b2': np.array(loaded_weights['b2'])
    }

except Exception as e:
    # Change your print statement to this:
    print(f"---> Could not load JSON. Actual Error: {e}")
...
def learning_process(X, Y, weights, input, hidden, output, epochs, learning_rate):
    #weights = initialize_parameters(input, hidden, output)
...
X = np.array([[0, 1, 1, 1],
              [0, 0, 1, 1]]) 
Y = np.array([[0, 1, 0, 0]])  #False False False False

trained_weights = learning_process(X, Y, weights, input, hidden, output, epochs=5000, learning_rate=0.5)
